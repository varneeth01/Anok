...
export function LaunchHype() {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-secondary/5" aria-hidden="true" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] sm:w-[800px] h-[400px] sm:h-[800px] bg-secondary/10 blur-[150px] rounded-full pointer-events-none" aria-hidden="true" />
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-secondary/50 bg-secondary/10 text-secondary font-bold text-xs sm:text-sm tracking-widest mb-6 sm:mb-8 uppercase">
            <span className="w-2 h-2 rounded-full bg-secondary animate-pulse" aria-hidden="true" /> V1.0 INCOMING
          </div>

          <h2 id="launch-heading" className="text-3xl sm:text-6xl md:text-8xl font-black mb-4 sm:mb-6 text-glow-purple">
          ...
          <div
            role="timer"
            aria-label={`Countdown: ${timeLeft.days} days, ${timeLeft.hours} hours, ${timeLeft.minutes} minutes, ${timeLeft.seconds} seconds until launch`}
            className="flex justify-center gap-2 sm:gap-4 md:gap-8 mb-10 sm:mb-16"
          >
            {units.map((unit, i) => (
              <div key={i} className="flex flex-col items-center">
                <div className="w-16 h-20 sm:w-24 sm:h-28 md:w-32 md:h-36 bg-card border border-white/10 rounded-xl sm:rounded-2xl flex items-center justify-center text-2xl sm:text-4xl md:text-6xl font-black font-mono shadow-xl glow-purple relative overflow-hidden">
                  <div className="absolute top-0 w-full h-1/2 bg-white/5 border-b border-white/5" aria-hidden="true" />
                  <span aria-hidden="true">{unit.value.toString().padStart(2, '0')}</span>
                </div>
          ...
          <div className="max-w-md mx-auto flex flex-col sm:flex-row bg-card/80 backdrop-blur-md sm:p-2 rounded-2xl sm:rounded-full border border-white/10 shadow-2xl overflow-hidden">
            <label htmlFor="waitlist-email" className="sr-only">Enter your email address to join the ANOK waitlist</label>
            <input
              id="waitlist-email"
              type="email"
            ...
            <a
              ...
              title="Join the ANOK waitlist on Discord"
              aria-label="Join the ANOK waitlist – redirects to Discord"
